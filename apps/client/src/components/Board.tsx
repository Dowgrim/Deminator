...
import { Users, Play, Crown, Skull, Trophy, Eye, RefreshCw } from 'lucide-react';
    ...
    <main className="flex-1 w-full h-full p-4 md:p-6 flex flex-col items-center justify-center bg-slate-950 overflow-hidden relative select-none">
                ...
                {Object.values(gameState.players)
                  ...
                  .map((player, idx) => {
                    ...
                    return (
                      <div
                        key={player.id}
                        className={`flex items-center justify-between p-3 rounded-xl border border-slate-800/60 bg-slate-950/40 relative overflow-hidden ${
                          isMePlayer ? 'ring-1 ring-slate-700/80 bg-slate-850/20' : ''
                        }`}
      ...
      {gameState.status !== 'waiting' && (
        <div className="absolute bottom-4 md:bottom-6 right-4 md:right-6 flex items-center gap-2 bg-slate-900/90 border border-slate-800 p-2 rounded-2xl shadow-xl z-20 select-none">
          {(gameState.status === 'won' || gameState.status === 'gameover') && !showResults && (
            <button
