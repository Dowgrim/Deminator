/**
 *
 *	La classe JPImage est un panel ayant une (ou plusieurs) image(s) peinte(s)
 *		en fond.
 *
 *	Permet:
 ...
 *		Eviter un débordement de mémoire d'image (si plusieurs fois la même image chargée)
 ...
 *			(Il faudra pour cela parametrer le layout du contenant du JPImage)
 *
 *
 *
 ...
 *
 * Version 2.0.0:
 * 		+	Plusieurs images possibles sur une meme JPImage (permet la superposition d'images transparentes)
...
public class JPImage extends JPanel {
	 ...
	 * Permet de ne pas a avoir à redimentionner plusieurs fois une même image inutilement (si l'image de dimension voulue est déjà en mémoire)
	 */
	 ...
	 * Permet de ne pas a avoir à effectuer plusieurs acces systèmes inutiles (si l'image est déjà chargée en mémoire)
	 */
	...
	//	//	//	//	//	CONSTRUCTEURS	//	//	//	//	//
	/**
	 * Constructeur par défaut (le JPImage est alors à sa création, similaire à un simple JPanel)
	 */
	public JPImage()
	{	cheminsImages = new ArrayList<String>();	}

	/**
	 * Constructeur de JPImage
	 * @param adrs Adresses des images (séparés par des virgules, la première étant celle de fond, la dernière celle par dessus toutes)
	 */
	public JPImage(final String ... adrs)
	{	cheminsImages = new ArrayList<String>();
		//	Ajouter les images en paramètre une a une
		for(String s : adrs)
			cheminsImages.add(s);
	}
	...
	{	try{
			//	Initialier le paramètre de retour
			Image image = null;
	...
	/**
	 * @return Liste contenant les chemins de toutes les images empilées sur le JPImage courant
	 */
	...
	/**
	 * @param keySet Toutes les images dont les chemins auront été passés en paramètres seront chargés dans l'application
	...
	{	//	Pour chaque chemin en paramètre
		for(String cheminImage : keySet)
			try{
				// Verifier si l'image n'à jamais été chargée en mémoire
