...
/**
 * La classe dem.util.JPImage est un panel ayant une (ou plusieurs) image(s) peinte(s)
 * en fond.
 * <p>
 * Permet:
 ...
 * (Il faudra pour cela parametrer le layout du contenant du dem.util.JPImage)
 * <p>
 * <p>
 * <p>
 ...
 * <p>
 * Version 2.0.0:
 * +	Plusieurs images possibles sur une meme dem.util.JPImage (permet la superposition d'images transparentes)
...
public class JPImage extends JPanel {
	 ...
	 */
	private static final HashMap<String, HashMap<Dimension, Image>> images = new HashMap<>();
	/**
	 ...
	 */
	private static final HashMap<String, Image> imagesOriginales = new HashMap<>();
	...
	/**
	 * Constructeur par défaut (le dem.util.JPImage est alors à sa création, similaire à un simple JPanel)
	 */
	public JPImage() { cheminsImages = new ArrayList<>(); }

	/**
	 * Constructeur de dem.util.JPImage
	 *
	...
	public JPImage(final String... adrs) {
		...
		cheminsImages.addAll(Arrays.asList(adrs));
	}
	...
	private Image trouverImageDimension(final String cheminImage, final Dimension dim) {
		try {
			...
			if(images.containsKey(cheminImage))
				...
				images.put(cheminImage, new HashMap<>());
	 ...
	 */
	public void addImage(final String chemin) {    //	contournement d'une erreur probable
		if(chemin == null)
			return;
	 ...
	 */
	public void rmImage(final String chemin) {    // Contournement d'une erreur possible
		if(chemin == null)
			return;
	 ...
	 */
	public void rmImages() { cheminsImages.clear(); }
	...
	/**
	 * @return Liste contenant les chemins de toutes les images empilées sur le dem.util.JPImage courant
	 */
	...
	public static void preloadImages(Iterable<String> keySet) {
		...
		for(String cheminImage : keySet) {
			try {
				...
				if(!imagesOriginales.containsKey(cheminImage)) {    //	Alors charger l'image et la placer en mémoire
